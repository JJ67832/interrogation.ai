document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateRatingData() {
    ...
    fetch('/api/structured-data')
      .then(res => res.json())
      .then(data => {
        // Aktualisiere das JSON-LD-Skript
  ...
  function saveToHistory(project, questions) {
    const entry = {
      id: Date.now(),
    ...
    let history = JSON.parse(localStorage.getItem('codingHelperHistory')) || [];
    history.unshift(entry);
    if (history.length > 10) history = history.slice(0, 10);
    localStorage.setItem('codingHelperHistory', JSON.stringify(history));
  ...
  function renderHistory() {
    ...
    const history = JSON.parse(localStorage.getItem('codingHelperHistory')) || [];
    ...
    history.forEach(entry => {
      ...
      entryElement.innerHTML = `
        <div class="history-header">
          ...
          <span class="history-project">${entry.project}</span>
        </div>
  ...
  function showLimitPopup(cooldownSeconds) {
    ...
    function updateCountdown() {
      ...
      if (remaining <= 0) {
        ...
        location.reload(); // Seite neu laden um Session zu aktualisieren
      } else {
        remaining--;
      }
    }
  ...
  async function submitForm() {
    ...
    const erwartung = document.getElementById('erwartung').value.trim();
    ...
    try {
      const res = await fetch('/api/ask', {
        ...
        body: JSON.stringify({ projekt, problem, erwartung, bisheriges })
      });
      ...
      if (fragenArray.length > 0) {
        saveToHistory(projekt, fragenArray);
      }

      // Aktualisiere Anfragezähler
  ...
  document.getElementById('clear-history').addEventListener('click', () => {
    localStorage.removeItem('codingHelperHistory');
    renderHistory();
  });
  ...
  fetch('/api/auth/status')
    ...
    .then(user => {
      if (user && !user.error) {
        // Verstecke Zähler für angemeldete Benutzer
